...
#define RANGE_SUBSWEEP_IDX 0U

#define LOW_FREQ_LOOPBACK_SUBSWEEP_IDX 0U
#define LOW_FREQ_RANGE_SUBSWEEP_IDX    1U
...
/** @brief Gets the previous element of the circular buffer */
static float circular_float_buffer_get_previous(const circular_float_buffer_t *cb);
...
static void sort_displacements_desc(acc_vibration_handle_t *handle, uint32_t displacement_idx, uint32_t insert_pos, acc_vibration_result_t *result);
...
acc_vibration_handle_t *acc_vibration_handle_create(const acc_vibration_config_t *config)
{
	...
	if (status)
	{
		...
		handle->time_series.write_first_element = true;

		if (config->sweeps_per_frame < config->time_series_length && handle->continuous_data_acquisition == false)
		{
			handle->time_series.capacity = config->sweeps_per_frame;
		}
		else
		{
                           ...
                           acc_vibration_result_t  *result)
{
	...
	result->peak_count = 0;
	for (uint8_t i = 0; i < ACC_VIBRATION_MAX_REPORTED_PEAKS; i++)
	{
		result->peak_displacements[i] = 0.0f;
		result->peak_frequencies[i]   = 0.0f;
	}
	...
	if (handle->low_frequency_enhancement_enabled)
	{
		...
		{
			uint16_t measure_src_idx  = (2U * frame_idx) + 1U;
			uint16_t loopback_src_idx = 2U * frame_idx;
...
static void circular_float_buffer_write_angle(circular_float_buffer_t *cb, float new_element)
{
	if (cb->write_first_element == false)
	{
		float unwrap_buf[2];

		unwrap_buf[0] = circular_float_buffer_get_previous(cb);
		unwrap_buf[1] = new_element;

		acc_algorithm_unwrap(unwrap_buf, 2U);

		cb->buffer[cb->write_idx] = unwrap_buf[1];
	}
	else
	{
		cb->buffer[cb->write_idx] = new_element;
		cb->write_first_element   = false;
	}
...
static float circular_float_buffer_get_previous(const circular_float_buffer_t *cb)
{
	// circular_float_buffer_get wraps around to the previous value when called
	// with cb->capacity - 1U as chronological_idx
	float previous_element = circular_float_buffer_get(cb, cb->capacity - 1U);
...
static bool validate_config(const acc_vibration_config_t *config)
{
	...
	if (config->time_series_length < 64U)
	{
		ACC_LOG_ERROR("time_series_length is too small");
		success = false;
	}

	if (config->time_series_length < config->sweeps_per_frame)
	{
		ACC_LOG_ERROR("time_series_length must be equal to or greater than sweeps_per_frame");
		success = false;
	}

	if (config->sweeps_per_frame <= 2U)
	{
		ACC_LOG_ERROR("sweeps_per_frame is too small");
		success = false;
	}
...
static bool translate_config(const acc_vibration_config_t *config, acc_config_t *sensor_config)
{
	...
	if (status)
	{
		...
		if (config->low_frequency_enhancement)
		{
			...
			// clang-format off
			acc_config_subsweep_start_point_set(sensor_config,       0,                    LOW_FREQ_LOOPBACK_SUBSWEEP_IDX);
			acc_config_subsweep_num_points_set(sensor_config,        1U,                   LOW_FREQ_LOOPBACK_SUBSWEEP_IDX);
			acc_config_subsweep_step_length_set(sensor_config,       1U,                   LOW_FREQ_LOOPBACK_SUBSWEEP_IDX);
			acc_config_subsweep_hwaas_set(sensor_config,             8U,                   LOW_FREQ_LOOPBACK_SUBSWEEP_IDX);
			acc_config_subsweep_receiver_gain_set(sensor_config,     10U,                  LOW_FREQ_LOOPBACK_SUBSWEEP_IDX);
			acc_config_subsweep_enable_tx_set(sensor_config,         true,                 LOW_FREQ_LOOPBACK_SUBSWEEP_IDX);
			acc_config_subsweep_phase_enhancement_set(sensor_config, false,                LOW_FREQ_LOOPBACK_SUBSWEEP_IDX);
			acc_config_subsweep_enable_loopback_set(sensor_config,   true,                 LOW_FREQ_LOOPBACK_SUBSWEEP_IDX);
			acc_config_subsweep_profile_set(sensor_config,           ACC_CONFIG_PROFILE_5, LOW_FREQ_LOOPBACK_SUBSWEEP_IDX);
			acc_config_subsweep_prf_set(sensor_config,               loopback_prf,         LOW_FREQ_LOOPBACK_SUBSWEEP_IDX);
			// clang-format on

			acc_config_num_subsweeps_set(sensor_config, 2);
			range_subsweep_idx = LOW_FREQ_RANGE_SUBSWEEP_IDX;
		}
		else
		{
			acc_config_num_subsweeps_set(sensor_config, 1);
			range_subsweep_idx = RANGE_SUBSWEEP_IDX;
		}

		acc_config_prf_t measure_prf = acc_algorithm_select_prf(config->measured_point, config->profile, ACC_APPROX_BASE_STEP_LENGTH_M);

		// clang-format off
		acc_config_subsweep_start_point_set(sensor_config,       config->measured_point, range_subsweep_idx);
		acc_config_subsweep_num_points_set(sensor_config,        1U,                     range_subsweep_idx);
		acc_config_subsweep_step_length_set(sensor_config,       1U,                     range_subsweep_idx);
		acc_config_subsweep_hwaas_set(sensor_config,             config->hwaas,          range_subsweep_idx);
		acc_config_subsweep_receiver_gain_set(sensor_config,     10U,                    range_subsweep_idx);
		acc_config_subsweep_enable_tx_set(sensor_config,         true,                   range_subsweep_idx);
		acc_config_subsweep_phase_enhancement_set(sensor_config, false,                  range_subsweep_idx);
		acc_config_subsweep_enable_loopback_set(sensor_config,   false,                  range_subsweep_idx);
		acc_config_subsweep_profile_set(sensor_config,           config->profile,        range_subsweep_idx);
		acc_config_subsweep_prf_set(sensor_config,               measure_prf,            range_subsweep_idx);
		// clang-format on
	}
...
static void sort_displacements_desc(acc_vibration_handle_t *handle, uint32_t displacement_idx, uint32_t insert_pos, acc_vibration_result_t *result)
{
	float displacement = handle->lp_displacements[displacement_idx];

	while ((insert_pos > 0) && (displacement > result->peak_displacements[insert_pos - 1]))
	{
		// Shift smaller peaks to the right (last one might fall off).
		result->peak_displacements[insert_pos] = result->peak_displacements[insert_pos - 1];
		result->peak_frequencies[insert_pos]   = result->peak_frequencies[insert_pos - 1];
		insert_pos--;
	}

	result->peak_displacements[insert_pos] = displacement;
	result->peak_frequencies[insert_pos]   = handle->frequencies[displacement_idx + handle->rfft_read_offset];
}
...
static void update_vibration_result(acc_vibration_handle_t *handle, acc_vibration_config_t *config, acc_vibration_result_t *result)
{
	...
	// Compare displacements to threshold (exclude first point as it does not form a peak)
	for (uint16_t i = 1U; i < handle->data_length; i++)
	{
		if (handle->lp_displacements[i] > handle->threshold[i])
		{
			if (peak_count < ACC_VIBRATION_MAX_REPORTED_PEAKS)
			{
				uint32_t insert_pos = peak_count;
				sort_displacements_desc(handle, i, insert_pos, result);
				peak_count++;
			}
			else if (handle->lp_displacements[i] > result->peak_displacements[ACC_VIBRATION_MAX_REPORTED_PEAKS - 1U])
			{
				// We have reached the peak limit, insert and drop smallest.
				uint32_t insert_pos = ACC_VIBRATION_MAX_REPORTED_PEAKS - 1U;
				sort_displacements_desc(handle, i, insert_pos, result);
			}
		}
	}
...
static float *get_zero_mean_time_series(acc_vibration_handle_t *handle)
{
	...
	{
		handle->zero_mean_time_series[i]  = circular_float_buffer_get(&handle->time_series, i);
		mean                             += handle->zero_mean_time_series[i];
	}
	...
	for (uint16_t i = 0; i < handle->time_series.capacity; i++)
	{
		handle->zero_mean_time_series[i] = handle->zero_mean_time_series[i] - mean;
	}
...
static void calculate_threshold(acc_vibration_handle_t *handle, acc_vibration_config_t *config)
{
	for (uint16_t i = 0; i < handle->data_length; i++)
	{
		handle->threshold[i] = acc_algorithm_calculate_cfar(handle->lp_displacements,
		                                                    handle->data_length,
		                                                    CFAR_WINDOW_LENGTH,
