...
#define SENSOR_ID                     (1U)
#define SENSOR_CALIBRATION_TIMEOUT_MS (1000U)
...
static bool swap_config(app_context_t *context, smart_presence_mode_t new_mode)
{
	...
	if (status)
	{
		...
		context->measure_timeout_ms = sleep_time_ms + 1000U;
...
static bool sensor_calibration(app_context_t *context)
{
	...
	for (uint16_t i = 0; !status && (i <= calibration_retries); i++)
	{
		...
		do
		{
			status = acc_sensor_calibrate(context->sensor,
			                              &cal_complete,
			                              &context->sensor_cal_result,
			                              context->buffer,
			                              context->current_context->buffer_size);
			...
			{
				status = acc_hal_integration_wait_for_sensor_interrupt(SENSOR_ID, SENSOR_CALIBRATION_TIMEOUT_MS);
			}
...
static bool detector_get_next(app_context_t *context, acc_detector_presence_result_t *detector_result)
{
	...
	if (!acc_hal_integration_wait_for_sensor_interrupt(SENSOR_ID, context->measure_timeout_ms))
	{
...
static void set_config(acc_ref_app_smart_presence_config_t *app_config, smart_presence_preset_config_t preset)
{
	...
	switch (preset)
	{
		...
		case SMART_PRESENCE_PRESET_CONFIG_MEDIUM_RANGE:
			...
			// Nominal config
			app_config->nominal_config.num_zones = 6;
